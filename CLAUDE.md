...
- Apply gates differ: network/storage/compute apply only via manual `workflow_dispatch` with the apply checkbox; dns/cloudflare auto-apply on push to `main`. Also a known, undecided inconsistency — don't silently "fix" it.
- `deploy.yml` and `destroy.yml` are legacy — they run Terraform at the repo root, where no `.tf` files remain.
